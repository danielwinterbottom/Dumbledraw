#!/usr/bin/env python
# -*- coding: utf-8 -*-
#example: python plot_shapes_mssm_for_paper.py -c "em" -i ../hep_data_things/shapes_new/shapes_postfit_bOnly_HighMass.root --era "combined"  --model-independent --embedding --fake-factor --combine-backgrounds --mass 1200 --cross-section-ggh "0.0031" --cross-section-bbh "0.001"  --output output/ --normalize-by-bin-width
import ROOT
    ...
    for channel in args.channels:
        ...
        for category in channel_categories[channel]:
            ...
            if args.linear == True:
                ...
                plot = dd.Plot(
                    [0.55, [0.3, 0.28]], style="ModTDR", r=0.04, l=0.14, width=600)
           ...
           # # get observed data and total background histograms
            ...
            if int(category) < 30 and int(category) > 2:
                ...
                plot.add_legend(width=0.40, height=0.335, pos=3)
            ...
            plot.legend(1).scaleTextSize(1.3)
            plot.legend(1).Draw()

            # draw additional labels
            #plot.DrawCMS(cms_sub="")
            supp=False
            if ('em' in args.channels and int(category) in [32,34,35,37]) or ('em' not in args.channels  and int(category) in [33,34,36,37]): supp=True
            print args.channels, int(category), supp
            plot.DrawCMS(preliminary=False,supp=supp)
            if "2016" in args.era:
